            ...
            <form role="form" id="save"  action="{{ route('addLKA') }}" method="post" enctype="multipart/form-data">
                ...
                <div class="form-group row">
                    <label for="contact_person" class="col-sm-4 col-form-label">{{ __('Jenis Report') }}</label>
                      <div class="col-sm-8">
                          <select class="form-control select2" style="width: 100%;" id="jenis_lka"
                              name="jenis_lka">
                              <option value="" disabled selected>Option Report</option>
                              <option value="Finish Good" {{ $jenis_lka == 'Finish Good' ? 'selected' : '' }}>Finish Good</option>
                              <option value="Stabilita" {{ $jenis_lka == 'Stabilita' ? 'selected' : '' }}>Stabilita</option>
                              <option value="Raw Material" {{ $jenis_lka == 'Raw Material' ? 'selected' : '' }}>Raw Material</option>
                              <option value="Mikrobiologi" {{ $jenis_lka == 'Mikrobiologi' ? 'selected' : '' }}>Mikrobiologi</option>
                          </select>
                          @error('jenis_lka')
