            ...
            <form method="GET" action="{{ route('lka_logbook_fg') }}">
                <div class="input-group col-2 mx-auto">
                    ...
                    <select class="form-control select2" style="width: 20px;" id="searchby" name="searchby">
                        ...
                        <option value="3" {{ request()->input('searchby') == 3 ? 'selected' : '' }}>Tahun Terbit
                        </option>
    ...
    <div class="card">
        <div class="card-header">
            <div class="container" style="display: flex; justify-content: space-between;">
                <div>
                    <a onclick="window.location.href='{{ route('ShowFormLKA', ['jenis_lka' => 'Mikrobiologi']) }}'"
    ...
    <div class="modal fade" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true"
        ...
        <div class="modal-dialog modal-xl">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    @if (count($products) > 0)
                        <form role="form" id="approve-form" action="{{ route('update_lka') }}" method="post"
                            ...
                            <div class="form-group row">
                                <label for="tahun_terbit"
                                    class="col-sm-4 col-form-label">{{ __('Tahun Terbit') }}</label>
                                <div class="col-sm-8">
                                    <select class="form-control " style="width: 100%;" id="tahun_terbit"
                                        name="tahun_terbit">
                                        ...
                                        @for ($tahun = $tahun_sekarang; $tahun >= $tahun_terbit_awal; $tahun--)
                                            <option value="{{ $tahun }}"
                                                {{ $data['tahun_terbit'] == $tahun ? 'selected' : '' }}>
                                                {{ $tahun }}</option>
                                        @endfor
                                    </select>
                                    @error('tahun_terbit')
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form role="form" id="approve-form" action="{{ route('export_lka') }}" method="post">
                        @csrf
                        <input type="hidden" name="jenis_lka" id="jenis_lka" value="Mikrobiologi">
                        <div class="form-group row">
