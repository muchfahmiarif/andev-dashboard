  ...
  <div class="row py-2">
    <div class="col-md-6">
       <nav class="page-breadcrumb">
          <ol class="breadcrumb">
            <li class="breadcrumb-item"><p  href="{{ url('#') }}"> <a style="color:#000000" href="/development-journey/mikrobiologi">Mikrobiologi</a> / <a style="color:#000000" href="/development-journey/mikrobiologi?nama_item={{ Request::get('nama_item') }}"> {{ Request::get('nama_item') }}</a> / <b> {{ Request::get('category') }}</b></p></li>
          </ol>
        </nav>
    </div>
  </div>
      ...
      <div class="card-body">
        <div class="container">
          <div>
           ...
           data-bs-target="#create_timeline"  style="width: 300px; text-align: center; margin-right: 10px;"><i class="fas fa-plus"></i> Tambah Timeline</a>
            </a>
          </div>
      </div>
      </div>
          ...
          <div id="content">
            <ul class="timeline">
              ...
              <li class="event" data-date="{{ $data_date }}">
                <div>
                  <div class="hstack">
                 <a style="cursor: pointer;" class="title text-decoration-none" onclick="window.location.href='mikrobiologi_timeline/detail?nama_item={{ $d->nama_item }}&category={{ $d->zat_aktif }}&time_line={{ $d->time_line }}'">{{ $d->time_line }}</a> 
                 ...
                 <p> {!! htmlspecialchars_decode($d->body) !!}</p>
                  <div>
                    <a title="Tambah Data" type="button"  class="btn btn-secondary btn-sm text-white" data-bs-toggle="modal"
                    data-bs-target="#update_timeline{{ $d->id_dev_tl }}"><i class="fas fa-edit"></i></a>
                    </a>
                    <a title="Delete Data" type="button"  class="btn btn-danger btn-sm text-white" data-bs-toggle="modal"
                    data-bs-target="#delete{{ $d->id_dev_tl }}"><i class="fas fa-trash"></i></a>
                  </div>
                </div>
              </li>
              <div class="modal fade" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true" id="delete{{ $d->id_dev_tl }}">
                <div class="modal-dialog modal-lg">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            @if (count($products) > 0)
                            <form role="form" id="delete" action="{{ route('delete_Timeline') }}" method="post">
                                @csrf
                                @method('delete')
                                <input type="hidden" name="id_delete" id="id_delete" value="{{ $d->id_dev_tl }}">
                                <div>
                                  ...
                                  <label for="status" class="col-sm-4 col-form-label">{{ __('Status') }}</label>
                                    <div class="col-sm-8">
                                        <select class="form-control" style="width: 100%;" id="status"
                                            ...
                                            <option value="Mencari Litelatur" {{ $d->status ==  "Mencari Litelatur" ? 'selected' : '' }}>Mencari Litelatur</option>
                                            <option value="Pending" {{ $d->status ==  "Pending" ? 'selected' : '' }}>Pending</option>
                                            <option value="Done" {{ $d->status ==  "Done" ? 'selected' : '' }}>Done</option>
                                            <option value="Work In Progress" {{ $d->status ==  "Work In Progress" ? 'selected' : '' }}>Work In Progress</option>
                                        </select>
                                        @error('category')
              ...
              <form role="form" id="approve-form" action="{{ route('addTimeline') }}" method="post">
                  ...
                  <input type="hidden" name="id" id="id" >
                    ...
                    <div class="form-group row">
                      ...
                      <div class="col-sm-8">
                            <input id="input-body" name="body" required style="opacity: 0; width: 0; position: absolute; top: 37%; left: 36%">
