    ...
    <nav class="page-breadcrumb">
        <ol class="breadcrumb">
            <li class="breadcrumb-item">
                <p href="{{ url('#') }}">
                    <a style="color:#000000" href="/development-journey/mikrobiologi">Mikrobiologi</a> /
                    <a style="color:#000000"
                        href="/development-journey/mikrobiologi?nama_item={{ Request::get('nama_item') }}">
                        {{ Request::get('nama_item') }}</a> /
                    <a style="color:#000000"
                        href="/development-journey/mikrobiologi/mikrobiologi_timeline?nama_item={{ Request::get('nama_item') }}&category={{ Request::get('category') }}">{{ Request::get('category') }}</a>
                    /
                    <b>{{ Request::get('time_line') }}</b>
                </p>
            </li>
        </ol>
    </nav>
            ...
            <div class="card">
                <div class="card-body">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        ...
                        <a title="Tambah Data" type="button" class="btn btn-secondary btn-sm text-white"
                            data-bs-toggle="modal" data-bs-target="#update_timeline"><i class="fas fa-edit"></i></a>
                    </div>
                    <div class="card">
                        <div class="card-body">
                            <p>{!! htmlspecialchars_decode($body->body) !!}</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="card-body">
                    <a title="Tambah Data" type="button" class="btn btn-secondary btn-sm text-white" data-bs-toggle="modal"
                        data-bs-target="#create_document" style="width: 300px; text-align: center; margin-right: 10px;"><i
                            class="fas fa-plus"></i> Tambah Document</a>
                    </a>
                                                    ...
                                                    <div class="modal-body">
                                                        @if (count($products) > 0)
                                                            <form role="form" id="delete"
                                                                action="{{ route('delete_document') }}" method="post">
                                                                @csrf
                                                                @method('delete')
            ...
            <div class="modal fade" id="update_doc{{ $d->id_dev_doc }}">
                <div class="modal-dialog modal-lg">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <form role="form" id="approve-form" action="{{ route('addDocument') }}" method="post"
                                enctype="multipart/form-data">
                                @csrf
                                <input type="hidden" name="id" id="id" value="{{ $data['pid'] }}">
                                <input type="hidden" name="jenis_journey" id="jenis_journey" value="Mikrobiologi">
                                <input type="hidden" name="nama_item" id="nama_item"
                                    value="{{ $data['pnama_item'] }}">
                                <input type="hidden" name="zat_aktif" id="zat_aktif"
                                    value="{{ $data['pzat_aktif'] }}">
                                <input type="hidden" name="time_line" id="time_line"
                                    value="{{ $data['ptime_line'] }}">
                                <div class="form-group row">
                                    <label for="judul" class="col-sm-4 col-form-label">{{ __('Judul') }}</label>
                                    <div class="col-sm-8">
                                        <input type="text" class="form-control" id="judul" name="judul"
                                            value="{{ $data['pjudul'] }}" required>
                                    </div>
                                </div>
                                <br>
                                <div class="form-group row">
                                    ...
                                    <div class="col-sm-8">
                                        <textarea type="text" class="form-control" id="deskripsi" name="deskripsi" value="{{ $data['pdeskripsi'] }}"
                                            required>{{ $data['pdeskripsi'] }}</textarea>
                                    </div>
                                </div>
                                <br>
                                <div class="form-group row">
                                    ...
                                    <div class="col-sm-8">
                                        <select class="form-control" style="width: 100%;" id="status" name="status"
                                            ...
                                            <option value="Work In Progress"
                                                ...
                                                Progress</option>
                                            <option value="Done" {{ $data['pstatus'] == 'Done' ? 'selected' : '' }}>
                                                Done</option>
                                        </select>
                                        @error('status')
                                ...
                                <div class="form-group row">
                                    ...
                                    <div class="col-sm-8">
                                        <textarea type="text" class="form-control" id="keterangan" name="keterangan" value="{{ $data['pketerangan'] }}"
                                            required>{{ $data['pketerangan'] }}</textarea>
                                    </div>
                                </div>
                                <br>
                                <div class="form-group row">
                                    <label for="nama_document"
                                        class="col-sm-4 col-form-label">{{ __('Nama Document') }}</label>
                                    <div class="col-sm-8">
                                        <input type="text" class="form-control" id="nama_document"
                                            name="nama_document" value="{{ $data['pnama_document'] }}" required>
                                    </div>
                                </div>
                                <br>
                                <div class="form-group row">
                                ...
                                <div class="d-flex justify-content-end">
                                    <button id="button-save" type="submit"
                                        class="btn btn-primary text-white">Tambahkan</button>
                                </div>
                            </form>
                        </div>
                    </div>
