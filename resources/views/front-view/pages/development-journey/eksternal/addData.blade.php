                ...
                <form role="form" id="save" action="{{ route('addLKA') }}" method="post"
                    ...
                    <div class="form-group row">
                        <label for="contact_person"
                            class="col-sm-4 col-form-label">{{ __('Category Develop Journey') }}</label>
                        <div class="col-sm-8">
                            <select class="form-control select2" style="width: 100%;" id="jenis_journey"
                                name="jenis_journey">
                                <option value="" disabled selected>Option Develop Journey</option>
                                <option value="Finish Good" {{ $jenis_journey == 'Finish Good' ? 'selected' : '' }}>Finish
                                    Good</option>
                                <option value="External" {{ $jenis_journey == 'External' ? 'selected' : '' }}>External
                                </option>
                                <option value="Raw Material" {{ $jenis_journey == 'Raw Material' ? 'selected' : '' }}>Raw
                                    Material</option>
                                <option value="Mikrobiologi" {{ $jenis_journey == 'Mikrobiologi' ? 'selected' : '' }}>
                                    Mikrobiologi</option>
                            </select>
                            @error('jenis_journey')
