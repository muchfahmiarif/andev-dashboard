            ...
            <nav class="page-breadcrumb">
                <ol class="breadcrumb">
                    <li class="breadcrumb-item">
                        <p href="{{ url('#') }}"> <a style="color:#000000"
                                href="/development-journey/eksternal">Eksternal</a> /<b> {{ Request::get('nama_item') }}</b>
                        </p>
                    </li>
                </ol>
            </nav>
                    ...
                    <select class="form-control select2" style="width: 20px;" id="nama_item" name="nama_item">
                        ...
                        @foreach ($selectProduct as $data)
                            <option value="{{ $data->nama_item }}"
                                {{ request()->input('nama_item') == $data->nama_item ? 'selected' : '' }}>
                    ...
                    <div class="container" style="display: flex; ">
                        <div>
                            <a title="Tambah Data" type="button" class="btn btn-secondary btn-sm text-white"
                                ...
                                style="width: 300px; text-align: center; margin-right: 10px;"><i class="fas fa-plus"></i>
                                Tambah Product</a>
                            </a>
                        </div>
                    </div>
            ...
            <div class="card text-center">
                <div class="card-body">
                    <div class="container" style="text-align: center; ">
                        <div>
                            <a title="Tambah Data" type="button" class="btn btn-secondary btn-sm text-white"
                                data-bs-toggle="modal" data-bs-target="#update_zatAktif{{ $addJourney->id_devlop }}"
                                style="width: 300px; text-align: center; margin-right: 10px;"><i class="fas fa-plus"></i>
                                Tambah Zat Aktif</a>
                            </a>
                        </div>
                    ...
                    <div class="modal-body">
                        <form role="form" id="approve-form" action="{{ route('addZatAktif') }}" method="post">
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-sm-8">
                                    <input type="text" class="form-control" id="zat_aktif" name="zat_aktif"
                                        value="{{ old('zat_aktif') }}" required>
                                </div>
                            </div>
                            <br>
            ...
            <div class="card">
                <div class="card-body">
                    <div class="container">
                        <div>
                            <a title="Tambah Data" type="button" class="btn btn-secondary btn-sm text-white"
                                data-bs-toggle="modal" data-bs-target="#create_zatAktif"
                                style="width: 300px; text-align: center; margin-right: 10px;"><i class="fas fa-plus"></i>
                                Tambah Zat Aktif</a>
                            </a>
                        </div>
                        ...
                        <div class="card-header d-flex justify-content-between align-items-center">
                            ...
                            <div>
                                <a title="Delete Data" type="button" class="btn btn-danger btn-sm text-white"
                                    data-bs-toggle="modal" data-bs-target="#delete{{ $d->id_devlop }}">
                                    <i class="fas fa-trash"></i>
                                </a>
                            </div>
                        </div>
                        <div class="card-body">
                            <h5 onclick="window.location.href='eksternal/eksternal_timeline?nama_item={{ $d->nama_item }}&category={{ $d->zat_aktif }}'"
                                style="cursor: pointer" class="card-title">{{ $d->zat_aktif }}</h5>
                            <p class="card-text">Nama Product : {{ $d->nama_item }} </p>
                            <a onclick="window.location.href='eksternal/eksternal_timeline?nama_item={{ $d->nama_item }}&category={{ $d->zat_aktif }}'"
                                class="btn btn-primary text-white">Lihat Timeline</a>
                        </div>
                    </div>
                </div>
            @else
            ...
            <div class="modal fade" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true"
                ...
                <div class="modal-dialog modal-lg">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            @if (count($products) > 0)
                                <form role="form" id="delete" action="{{ route('delete_product') }}"
                                    method="post">
                                    @csrf
                                    @method('delete')
