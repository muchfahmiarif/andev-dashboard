...
<div class="row py-2">
  <div class="col-md-5">
    <h4>
      <span class="text-muted fw-light">Tables /</span> Mikrobiologi
    </h4>
  </div>
    ...
    <form method="GET" action="{{ route('logbook_fg') }}">
        <div class="input-group col-2 mx-auto">
            ...
            <select class="form-control select2" style="width: 20px;" id="searchby" name="searchby">
                ...
                <option value="2" {{ request()->input('searchby') == 2 ? 'selected' : '' }}>Kode Racikan</option>
...
<div class="card">
    <div class="card-header">
        <div class="container" style="display: flex; justify-content: space-between;">
            <div>
                <a onclick="window.location.href='{{ route('ShowFormReportAnalysis', ['jenis_report' => 'Mikrobiologi']) }}'" class="btn btn-primary text-white" style="width: 200px; text-align: center; margin-right: 10px;">
                  ...
                  @if (count($products) > 0)
                      @foreach ($products as $key => $d)
                          ...
                          <tr>
                            ...
                            <td style="min-width: 100px"class="text-center align-middle ">
                                @php
                                $tgl_analisa = isset($data['tgl_analisa']) ? Carbon\Carbon::parse($data['tgl_analisa']) : null;
                                $tgl_selesai = isset($data['tgl_selesai']) ? Carbon\Carbon::parse($data['tgl_selesai']) : null;
                                @endphp
                            ...
                            <td style="min-width: 100px"class="text-center align-middle ">
                              @if ($data['status'] == 'MS')
                                        <a title="Priority" class="badge rounded-pill-lg text-white bg-danger"> {{ $data['status'] }}</a>
                                    @elseif ($data['status'] == 'On Progress')
                                        <a title="Priority" class="badge rounded-pill-lg text-white bg-warning"> {{ $data['status'] }}</a>
                                    @else
                                        <a title="Priority" class="badge rounded-pill-lg text-white bg-primary"> {{ $data['status'] }}</a>
                                    @endif
                            </td>
                            <td style="min-width: 100px"class="text-center align-middle ">
                                    @php
                                    $tgl_selesai_report = isset($data['tgl_selesai_report']) ? Carbon\Carbon::parse($data['tgl_selesai_report']) : null;
                                    $tgl_masuk = isset($data['tgl_masuk']) ? Carbon\Carbon::parse($data['tgl_masuk']) : null;
                                @endphp
                            ...
                            <td style="min-width: 100px"class="text-center align-middle ">
                                ...
                                data-bs-target="#remarks{{ $d->id_report }}">Remarks</a>
                            </td>
                              <td style="min-width: 100px;"class="text-center align-middle ">
                                    <a title="Tambah Scar" type="button"  class="btn btn-secondary btn-sm text-white" data-bs-toggle="modal"
                                    data-bs-target="#update_data{{ $d->id_report }}"><i class="fas fa-edit"></i></a>
                                    <a title="Tambah Scar" type="button"  class="btn btn-danger btn-sm text-white" data-bs-toggle="modal"
                                    data-bs-target="#delete{{ $d->id_report }}"><i class="fas fa-trash"></i></a>
                              </td>
                              
                          </tr>
                          <div class="modal fade" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true" id="remarks{{ $d->id_report }}">
                          ...
                          <div class="modal fade" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true" id="delete{{ $d->id_report }}">
                            <div class="modal-dialog modal-lg">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h5 class="modal-title" id="exampleModalLabel">Action</h5>
                            ...
                            <div class="modal fade" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true" id="update_data{{ $d->id_report }}">
                                <div class="modal-dialog modal-xl">
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-body">
                                            @if (count($products) > 0)
                                            <form role="form" id="approve-form" action="{{ route('update_report') }}" method="post" enctype="multipart/form-data">
                                            @csrf
                                                ...
                                                <label for="contact_person" class="col-sm-4 col-form-label">{{ __('Category') }}</label>
                                                  <div class="col-sm-4">
                                                      <select class="form-control select2" style="width: 100%;" id="category"
                                                          ...
                                                          <option value="Obat" {{ $data['category'] == 'Obat' ? 'selected' : '' }}>Obat</option>
                                                          <option value="Kosmetik" {{ $data['category'] == 'Kosmetik' ? 'selected' : '' }}>Kosmetik</option>
                                                          <option value="Eksternal" {{ $data['category'] == 'Eksternal' ? 'selected' : '' }}>Eksternal</option>
                                                          <option value="RM Alternatif" {{ $data['category'] == 'RM Alternatif' ? 'selected' : '' }}>RM Alternatif</option>
                                                          <option value="New RM" {{ $data['category'] == 'New RM' ? 'selected' : '' }}>New RM</option>
                                                          <option value="Efektivitas Pengawet" {{ $data['category'] == 'Efektivitas Pengawet' ? 'selected' : '' }}>Efektivitas Pengawet</option>
                                                      </select>
                                                      @error('category')
                                            ...
                                            <div class="form-group row">
                                                ...
                                                <div class="col-sm-4">
                                                    <input type="text" class="form-control" id="kode_racikan" name="kode_racikan" value="{{ $data['kode_racikan'] }}">
                                                </div>
                                                <label for="no_bets" class="col-sm-1 col-form-label">{{ __('No Bets') }}</label>
                                                <div class="col-sm-3">
                                                    <input type="text" class="form-control" id="no_bets" name="no_bets"  value="{{ $data['no_bets'] }}">
                                                </div>
                                            </div>
                                            <br>
                                            <div class="form-group row">
                                                ...
                                                <div class="col-sm-8">
                                                    ...
                                                    <option value="In Used H-0" {{ $data['bulan'] == 'In Used H-0' ? 'selected' : '' }}>In Used H-0</option>
                                                    <option value="In Used H-3" {{ $data['bulan'] == 'In Used H-3' ? 'selected' : '' }}>In Used H-3</option>
                                                    <option value="In Used H-7" {{ $data['bulan'] == 'In Used H-7' ? 'selected' : '' }}>In Used H-7</option>
                                                    <option value="In Used H-14" {{ $data['bulan'] == 'In Used H-14' ? 'selected' : '' }}>In Used H-14</option>
                                                    <option value="In Used H-21" {{ $data['bulan'] == 'In Used H-21' ? 'selected' : '' }}>In Used H-21</option>
                                                    <option value="In Used H-28" {{ $data['bulan'] == 'In Used H-28' ? 'selected' : '' }}>In Used H-28</option>
                                                    <option value="In Used H-35" {{ $data['bulan'] == 'In Used H-35' ? 'selected' : '' }}>In Used H-35</option>
                                                    <option value="In Used H-49" {{ $data['bulan'] == 'In Used H-49' ? 'selected' : '' }}>In Used H-49</option>
                                                    <option value="In Used H-63" {{ $data['bulan'] == 'In Used H-63' ? 'selected' : '' }}>In Used H-63</option>
                                                    <option value="B-0" {{ $data['bulan'] == 'B-0' ? 'selected' : '' }}>B-0</option>
                                                    <option value="B-1" {{ $data['bulan'] == 'B-1' ? 'selected' : '' }}>B-1</option>
                                                    <option value="B-2" {{ $data['bulan'] == 'B-2' ? 'selected' : '' }}>B-2</option>
                                                    <option value="B-3" {{ $data['bulan'] == 'B-3' ? 'selected' : '' }}>B-3</option>
                                                    <option value="B-4" {{ $data['bulan'] == 'B-4' ? 'selected' : '' }}>B-4</option>
                                                    <option value="B-5" {{ $data['bulan'] == 'B-5' ? 'selected' : '' }}>B-5</option>
                                                    <option value="B-6" {{ $data['bulan'] == 'B-6' ? 'selected' : '' }}>B-6</option>
                                                    <option value="B-9" {{ $data['bulan'] == 'B-9' ? 'selected' : '' }}>B-9</option>
                                                    <option value="B-12" {{ $data['bulan'] == 'B-12' ? 'selected' : '' }}>B-12</option>
                                                    <option value="B-18" {{ $data['bulan'] == 'B-18' ? 'selected' : '' }}>B-18</option>
                                                    <option value="B-24" {{ $data['bulan'] == 'B-24' ? 'selected' : '' }}>B-24</option>
                                                    <option value="B-36" {{ $data['bulan'] == 'B-36' ? 'selected' : '' }}>B-36</option>
                                                </select>
                                            </div>
                                            @error('bulan')
                                            <div class="text-danger">{{ $message }}</div>
                                            ...
                                            <div class="form-group row">
                                                ...
                                                <div class="col-sm-4">
                                                        <input type="text" class="form-control" id="parameter" name="parameter" value="{{ $data['parameter'] }}">
                                                </div>
                                                 <label for="storage" class="col-sm-1 col-form-label">Storage</label>
                                                     <div class="col-sm-4">
                                                        <input type="text" class="form-control" id="storage" name="storage" value="{{ $data['storage'] }}">
                                                    </div>
                                            </div>
                                            <br>
                                            <div class="form-group row">
                                            ...
                                            <div class="form-group row">
                                                ...
                                                <div class="col-sm-8">
                                                    <input type="date" class="form-control" id="tgl_analisa" name="tgl_analisa"   value="{{ $data['tgl_analisa'] }}">
                                                </div>
                                            </div>
                                            <br>
                                            <div class="form-group row">
                                                ...
                                                <div class="col-sm-8">
                                                    <input type="date" class="form-control" id="tgl_est" name="tgl_est"   value="{{ $data['tgl_est'] }}">
                                                </div>
                                            </div>
                                            <br>
                                            <div class="form-group row">
                                            ...
                                            <br>
                                            <div class="form-group row">
                                                <label for="tgl_selesai_report" class="col-sm-4 col-form-label">{{ __('Tgl Selesai Report') }}</label>
                                                <div class="col-sm-8">
                                                    <input type="date" class="form-control" id="tgl_selesai_report" name="tgl_selesai_report"  value="{{ $data['tgl_selesai_report'] }}">
                                                </div>
                                            </div>
                                            <br>
                                            <div class="form-group row">
                                                ...
                                                <div class="col-sm-8">
                                                    ...
                                                    <option value="Achived" {{ $data['status_lt'] == 'Achived' ? 'selected' : '' }}>Achived</option>
                                                    <option value="Not Reached" {{ $data['status_lt'] == 'Not Reached' ? 'selected' : '' }}>Not Reached</option>
                                                </select>
                                             </div>
                                            </div>
                                            <br>
                                            <div class="form-group row">
                                                ...
                                                <div class="col-sm-8">
                                                    <select class="form-control select2" style="width: 100%;" id="nama_analis" name="nama_analis" value="{{ $data['nama_analis'] }}">
                                                    <option value="" disabled selected>Select Analis</option>
                                                    <option value="Shiddiq" {{ $data['nama_analis'] == 'Shiddiq' ? 'selected' : '' }}>Shiddiq</option>
                                                    <option value="Dimas" {{ $data['nama_analis'] == 'Dimas' ? 'selected' : '' }}>Dimas</option>
                                                    <option value="Wahyu" {{ $data['nama_analis'] == 'Wahyu' ? 'selected' : '' }}>Wahyu</option>
                                                    <option value="Daffa" {{ $data['nama_analis'] == 'Daffa' ? 'selected' : '' }}>Daffa</option>
                                                    <option value="Luthfi" {{ $data['nama_analis'] == 'Luthfi' ? 'selected' : '' }}>Luthfi</option>
                                                    <option value="Tiwi" {{ $data['nama_analis'] == 'Tiwi' ? 'selected' : '' }}>Tiwi</option>
                                                    <option value="Rica" {{ $data['nama_analis'] == 'Rica' ? 'selected' : '' }}>Rica</option>
                                                </select>
                                             </div>
                                            </div>
                                            <br>
                                            <div class="form-group row">
                                                ...
                                                <div class="col-sm-8">
                                                    @if($data['file_upload'])
                                                        <p>File yang sudah diunggah:  <a href="{{ asset('storage/' . $data['file_upload']) }}" target="_blank">Lihat Gambar</a>
                                                    @endif
                                            ...
                                            <div class="form-group row">
                                                ...
                                                <div class="col-sm-8">
                                                    <select class="form-control select2" style="width: 100%;" id="status" name="status" value="{{ $data['status'] }}">
                                                    <option value="" disabled selected>Status</option>
                                                    <option value="MS" {{ $data['status'] == 'MS' ? 'selected' : '' }}>MS</option>
                                                    <option value="TMS" {{ $data['status'] == 'TMS' ? 'selected' : '' }}>TMS</option>
                                            ...
                                            <div class="form-group row">
                                                ...
                                                <div class="col-sm-8">
                                                    <textarea type="text"  class="form-control" id="keterangan" name="keterangan" placeholder="Masukan Keterangan"  value="{{ $data['keterangan'] }}">{{ $data['keterangan'] }}</textarea>
                                                    @error('keterangan')
                                                    <div class="text-danger">{{ $message }}</div>
                                                    @enderror
                                                </div>
                                            ...
                                            <div class="d-flex justify-content-end">
                                                <a href="{{ route('logbook_fg') }}" type="button" class="btn btn-default mr-3">{{ __('Batal') }} </a>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form role="form" id="approve-form" action="{{ route('export_report') }}" method="post">
                        @csrf
                        <input type="hidden" name="jenis_report" id="jenis_report" value="Mikrobiologi">
                        <div class="form-group row">
